...
export const useInactivityLogout = (): UseInactivityLogoutReturn => {
  ...
  const { accessToken, logout, lastActivityTimestamp, setLastActivityTimestamp, logoutReason, setLogoutReason } = useAuthStore();
  
  const timeoutRef = useRef<NodeJS.Timeout | null>(null);
  const INACTIVITY_TIMEOUT = 2 * 1000; // 10 segundos en milisegegundos

  const [isModalOpen, setIsModalOpen] = useState(false); // Estado interno del modal en el hook

  // Función memoizada para ejecutar el cierre de sesión por inactividad
  ...
  // Función memoizada para iniciar/reiniciar el temporizador de inactividad
  const startInactivityTimer = useCallback(() => {
    if (timeoutRef.current) {
      clearTimeout(timeoutRef.current);
    }
    timeoutRef.current = setTimeout(performLogoutDueToInactivity, INACTIVITY_TIMEOUT);
  }, [performLogoutDueToInactivity]);

  // Función memoizada para manejar los eventos de actividad del usuario
  ...
  }, [setLastActivityTimestamp, startInactivityTimer]);

  // Efecto principal para manejar la lógica de inactividad y los listeners
  useEffect(() => {
    ...
    if (accessToken) {
      ...
      if (lastActivityTimestamp) {
        ...
        if (timeSinceLastActivity > INACTIVITY_TIMEOUT) {
          console.log("Sesión expirada por inactividad (al cargar la app), cerrando sesión...");
          logout('inactivity');
          navigate('/login', { replace: true });
          setIsModalOpen(true);
      ...
      // 2. Inicia el temporizador de inactividad al principio del efecto
      // Esto cubre el caso inicial cuando el usuario se loguea o reabre la app y la sesión es válida
      startInactivityTimer();

      // Añade listeners para eventos de actividad global
  ...
  }, [accessToken, navigate, logout, lastActivityTimestamp, setLastActivityTimestamp, startInactivityTimer, handleUserActivity, setLogoutReason]);
  ...
  }, [logoutReason, accessToken, setLogoutReason]);
